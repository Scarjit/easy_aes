...
mod tests {
    ...
    fn generate_tests() -> Vec<(PlainText, Key)> {
        let tests: Vec<(PlainText,Key)> = (0..NUMBER_OF_TESTS).into_par_iter().map(|_|{
            ...
            let tx = iter::repeat(())
                ...
                .collect::<String>();
            let tk = iter::repeat(())
                .map(|()| rng.clone().sample(Alphanumeric)).map(char::from).take(32).collect::<String>();
            (tx,tk)
        }).collect();
        tests
    }
